...
struct Utils utils_initializer() {
    ...
    utils.sleeping_barbers = semget(IPC_PRIVATE, 1, IPC_CREAT | IPC_EXCL | 0600); // Holds info on how many barbers are free
    ...
    utils.customers_processing = semget(IPC_PRIVATE, n_of_customers, IPC_CREAT | IPC_EXCL | 0600); // Holds info about giving haircut to every customer
