...
void bear_attack(struct Utils *utils) {
    ...
    if (left_to_kill > 0) {
        for (i = utils->game_status->warriors - 1; i >= 0; i--) {
            kill(utils->bees[WARRIOR][i], SIGKILL);
        ...
        for (i = 0; i < N_OF_BEES_KILLING_BEAR; i++) {
            kill(utils->bees[WARRIOR][ptr], SIGKILL);
    ...
    if (!looses) {
        ...
        if (left_to_kill > utils->game_status->workers) {
            for (i = utils->game_status->workers - 1; i >= 0; i--) {
                kill(utils->bees[WORKER][i], SIGKILL);
            ...
            for (i = 0; i < left_to_kill; i++) {
                kill(utils->bees[WORKER][ptr], SIGKILL);
        ...
        if(utils->game_status->honey < 0)
            utils->game_status->honey = 0;
    }

    printf("Bear looses %d \n", looses);
    game_check(utils); /* Check if you lose game */
}
